      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-30">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center space-x-2 cursor-pointer" onClick={() => navigate('/app/learn')}>
              <div className="w-8 h-8 bg-purple-600 rounded-lg flex items-center justify-center text-white shadow-purple-200 shadow-md">
                <Languages size={18} />
              </div>
              <span className="text-lg font-bold tracking-tight text-slate-900 hidden sm:block">Lingual</span>
            </div>
            ...
            <div className="hidden md:flex items-center space-x-1 bg-slate-100 rounded-full px-3 py-1.5 ml-6 border border-slate-200 hover:border-purple-200 cursor-pointer transition-colors">
              ...
              <span className="text-sm font-semibold text-slate-700">Spanish</span>
            </div>
          </div>
             ...
             <DropdownMenu.Root>
               <DropdownMenu.Trigger asChild>
                 <button className="flex items-center gap-2 pl-2 rounded-full hover:bg-slate-50 transition-colors focus:outline-none focus:ring-2 focus:ring-purple-200">
                   <img src={USER_AVATAR} alt="User" className="w-9 h-9 rounded-full border-2 border-white shadow-sm object-cover" />
                   <div className="hidden lg:block text-left mr-2">
                     <div className="text-sm font-semibold text-slate-900 leading-none">Michael C.</div>
               ...
               <DropdownMenu.Portal>
                 <DropdownMenu.Content className="min-w-[200px] bg-white rounded-xl shadow-xl border border-slate-100 p-2 z-50 animate-in fade-in zoom-in-95 duration-200" align="end" sideOffset={5}>
                   <DropdownMenu.Item className="flex items-center px-3 py-2 text-sm text-slate-700 rounded-lg hover:bg-purple-50 hover:text-purple-700 cursor-pointer outline-none" onClick={() => navigate('/app/profile')}>
                     <User size={16} className="mr-2" /> Profile
                   </DropdownMenu.Item>
                   <DropdownMenu.Item className="flex items-center px-3 py-2 text-sm text-slate-700 rounded-lg hover:bg-purple-50 hover:text-purple-700 cursor-pointer outline-none" onClick={() => navigate('/app/settings')}>
                     <Settings size={16} className="mr-2" /> Settings
                   </DropdownMenu.Item>
                   <DropdownMenu.Item className="flex items-center px-3 py-2 text-sm text-slate-700 rounded-lg hover:bg-purple-50 hover:text-purple-700 cursor-pointer outline-none" onClick={() => navigate('/app/learn')}>
                     <BookOpen size={16} className="mr-2" /> Learning
                   </DropdownMenu.Item>
                   <DropdownMenu.Separator className="h-px bg-slate-100 my-1" />
                   <DropdownMenu.Item className="flex items-center px-3 py-2 text-sm text-red-600 rounded-lg hover:bg-red-50 cursor-pointer outline-none" onClick={handleLogout}>
        ...
        <div className="fixed inset-0 z-40 bg-slate-900/20 backdrop-blur-sm md:hidden" onClick={() => setIsMobileMenuOpen(false)}>
           ...
           >
             ...
             <nav className="space-y-2">
               {[
                 { icon: BookOpen, label: 'Learning', path: '/app/learn' },
                 { icon: User, label: 'Profile', path: '/app/profile' },
                 { icon: Settings, label: 'Settings', path: '/app/settings' },
                 { icon: LayoutDashboard, label: 'Teacher View', path: '/app/teacher' },
               ].map((item) => (
                 <NavLink 
                   key={item.path}
                   to={item.path}
                   ...
                   className={({ isActive }) => `
                     ...
                     ${isActive ? 'bg-purple-50 text-purple-700 font-semibold' : 'text-slate-600 hover:bg-slate-50'}
                   `}
                 >
                   <item.icon size={20} />
