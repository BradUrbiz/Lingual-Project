...
- `pedagogy/` — the **Pedagogy Engine** (S1 thin spine + S2 closed-loop recycling + S3.1 post-task coach review). `policies.py` (teacher-policy normalizers, relocated from the resolver), `routing.py` (target-type-aware feedback routing — the behaviour win), `plan.py` (`compile_prompt_plan` → `PromptPlan` + `serialize_plan_preview`), `coverage.py` (stdlib-only S2 reader: `compute_coverage_state(...) → CoverageState` — count-only tiering of expression+vocabulary targets, no DB/OpenAI imports), `coach_review.py` (stdlib-only S3.1 pure module: `CoachReview` schema + `build_coach_review_prompt` / `parse_coach_review` / `serialize_coach_review`, `feedback_mode`-keyed `work_on` depth caps — no DB/OpenAI imports), `render/assignment_prompt.py` (`render_assignment_prompt(plan, surface)` + the recycling section), `integration.py` (flag-gated dispatch + `recycling_enabled()` / `coach_review_enabled()` env helpers). **Import boundary (enforced + tested):** `plan.py`/`routing.py`/`coverage.py`/`coach_review.py` import no OpenAI/Canvas/resolver — verified by `test_pedagogy_engine_s1.ImportBoundaryTestCase`. The S3.1 **impure** orchestrator lives outside the package at `backend/services/coach_review_service.py` (`generate_coach_review(deps, bootstrap, uid, session_id) → dict | None`: transcript fetch + OpenAI call + fail-open snapshot into `analysis_state['coach_review']`), behind `GET /api/practice-sessions/<id>/coach-review` (generate-on-read, cached). **Flag state:** S1 assignment render is now **unconditional** (the `PEDAGOGY_ENGINE_ASSIGNMENT_RENDER` flag + legacy `build_assignment_system_prompt` were retired 2026-06-23); the S2 recycling loop is gated by `PEDAGOGY_ENGINE_RECYCLING` (LIVE, cloudbuild default `'1'`, route compute fail-open); the S3.1 coach review is gated by `PEDAGOGY_ENGINE_COACH_REVIEW` (default off — gate runs before any read/LLM, fail-open to `coachReview: null`). See `docs/school-integration/Pedagogy Engineering/`.
