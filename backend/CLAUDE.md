...
- `pedagogy/` — the **Pedagogy Engine** (S1 thin spine + S2 closed-loop recycling). `policies.py` (teacher-policy normalizers, relocated from the resolver), `routing.py` (target-type-aware feedback routing — the behaviour win), `plan.py` (`compile_prompt_plan` → `PromptPlan` + `serialize_plan_preview`), `coverage.py` (stdlib-only S2 reader: `compute_coverage_state(...) → CoverageState` — count-only tiering of expression+vocabulary targets, no DB/OpenAI imports), `render/assignment_prompt.py` (`render_assignment_prompt(plan, surface)` + the recycling section), `integration.py` (flag-gated dispatch). **Import boundary (enforced + tested):** `plan.py`/`routing.py`/`coverage.py` import no OpenAI/Canvas/resolver — verified by `test_pedagogy_engine_s1.ImportBoundaryTestCase`. Gated by `PEDAGOGY_ENGINE_ASSIGNMENT_RENDER` (default off → legacy `build_assignment_system_prompt`); the S2 recycling loop is additionally gated by `PEDAGOGY_ENGINE_RECYCLING` (default off; needs BOTH flags on, route compute fail-open). See `docs/school-integration/Pedagogy Engineering/`.
