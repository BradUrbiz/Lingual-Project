...
**Product translation:** the coach track should surface to learners as a compact **Conversation Sidecar**, not as inline interruptions in the main conversation. The sidecar has a **Feedback / Ask** toggle: Feedback owns correction chips, target reminders, self-repair prompts, ASR caveats, and the post-task review queue; Ask owns quick contextual help ("what was that word?", "how do I say X?", replay, clarification, translation, hint). Ask responses are scaffolds, not evidence of learner production, so they must be logged separately and excluded from target-coverage claims unless the learner later produces the form in the main conversation.
...
> **Because gpt-realtime-mini's instruction-adherence is low, the live voice prompt must do *less*, not more.** The Conversation Sidecar / coach track isn't just a feature — it's the mechanism that lets the main prompt shed its correction rules so the few rules it keeps (turn cap, language lock, always-invite-production) actually get followed.

That reframes the phasing slightly: the Conversation Sidecar / coach track is arguably *more* load-bearing than the spec treats it, precisely because of the mini model. The current architecture keeps S1 narrow but makes S3 the first real conversation-management surface, with the main tutor and curator/sidecar layer deliberately separated.
