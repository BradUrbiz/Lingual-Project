...
Date: 2026-06-03 | Status: **Slice A (assignments) SHIPPED & PG-AUTHORITATIVE. Slice B (practice_sessions dual-write) SHIPPED & LIVE. Slice C (learning_events dual-write) SHIPPED & LIVE in prod (2026-06-03, rev `lingual-app-00058-gx4`, `DUAL_WRITE_ANALYTICS_EVENTS=1`): `shadow_write_turn` + the §5b.2-#7 self-disable live; events term-scope backfill **1401 inserted / 0 errors**; Pass 2 per-session event-COUNT parity **`in_sync`, fs 1401 = pg 1401, 0 short / 0 surplus**. Slice D (analytics read adapters + split-pass parity) SHIPPED — **SESSION-read FLIPPED TO PG-AUTHORITATIVE** (2026-06-03, rev `lingual-app-00059-msz`, `READ_PG_ANALYTICS_SESSIONS=1`, fail-open); EVENT-read still `'0'` (gated on the GIN). Slice E (read FLIPS + Firestore write retirement) IN PROGRESS — session reads now off Firestore. **Verification:** Pass 1 session_summary parity `in_sync` 49/49 0 mismatched (incl. 1 LIVE non-backfilled smoke-test session — `shadow_write_turn` proven end-to-end through the route across session-create/events/finalize seams), Pass 2 event-COUNT `in_sync` 1408=1408, a live shadow compare 1/0, post-flip teacher analytics PG-served (summary.totals match the live session) with 0 fail-open. Remaining: apply Alembic `0003` GIN to prod → `READ_PG_ANALYTICS_EVENTS=shadow`→`=1` → `WRITE_FIRESTORE_ANALYTICS` retirement (code not yet built). Deploy chain: rev 00057 (C+D dormant) → 00058 (`DUAL_WRITE_ANALYTICS_EVENTS=1` + `READ_PG_ANALYTICS_SESSIONS=shadow`) → 00059 (`READ_PG_ANALYTICS_SESSIONS=1`).**
