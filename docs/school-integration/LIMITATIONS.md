...
31. **Plan 4 follow-ups.** Notable shipped-state constraints on the teacher
    ...
    - Search excludes suspended and archived orgs. _RESOLVED 2026-05-23._ The `status=='active'` filter is now applied in the Firestore query (`search_organizations` chains `where('status','==','active')` before the `name_lower` range and `limit`), so the limit is applied after filtering and active orgs are no longer dropped behind inactive ones. Covered by the existing composite index `organizations (status ASC, name_lower ASC)` and the emulator test `TestOrganizationSearchCorrectness`.
