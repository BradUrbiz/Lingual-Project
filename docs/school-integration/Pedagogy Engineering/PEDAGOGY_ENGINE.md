...
|---|---|---|---|---|
| **S1 — Thin spine** ✅ BUILT + CUT OVER 2026-06-22 (`PEDAGOGY_ENGINE_ASSIGNMENT_RENDER=1` LIVE in prod, rev 00067-m6j; cloudbuild default still `'0'` and legacy builder retained pending burn-in; see `PEDAGOGY_ENGINE_S1.md` §10 + LIMITATIONS #53) | Reborn `backend/services/pedagogy/`: `compile_prompt_plan(bootstrap) -> PromptPlan` (§4.2a — targets + feedback_policy + task_context only) and `render_assignment_prompt(plan, surface)`. Re-house **only the assignment path** over the plan; **do not touch free practice**; prove zero prompt regression via snapshot; ship **one** visible behavior win (grammar-target slips escalate per `feedbackPolicy.mode`). Detailed in `PEDAGOGY_ENGINE_S1.md`. | L2 (thin), L5 (routing only) + render-helper | `system_prompt` (assignment only) | new backend module + tests + eval harness (not "zero" — see note) |
| **S2 — Closed loop** ✅ BUILT 2026-06-23 (behind `PEDAGOGY_ENGINE_RECYCLING`, cloudbuild default `'0'`; INERT until the S1 render flag is also live — gated on BOTH flags; **not yet cut over**; see `PEDAGOGY_ENGINE_S2.md` + LIMITATIONS #53) | Coverage/evidence reader over existing `learning_events`; L3 learner-model accumulates **heuristic coverage tiers (hit-count proxy: not_attempted = 0 hits / emerging = 1–2 / solid = ≥3)** + repeated-error patterns (no affect yet) — count-based coverage evidence, **NOT model-verified mastery** (see LIMITATIONS #53(g)); feeds recycling + uncovered-target awareness back into S1's plan. Coverage tiering is **expression + vocabulary targets only** (they have real per-target hit maps in `session_summary`); grammar rules + objectives stay on S1 routing. The route coverage compute is **fail-open** (any reader/compute failure → no recycling, never a live-path 500). | L3 (partial), L2 coverage | `system_prompt` | reader only (no new store) |
