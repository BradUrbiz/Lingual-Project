...
| **S4 — Affect + debrief** — **S4.1 BUILT behind `PEDAGOGY_ENGINE_AFFECT` (default `'0'`), NOT yet cut over** (see `PEDAGOGY_ENGINE_S4.md`); **S4.2 (evidence-backed teacher debrief) still pending** | **S4.1:** coarse readiness heuristic (turn-length trend + repair density + abandonment) → `AffectState` → gentler L5 stance lines when strained; pure `affect.py` (stdlib-only: `compute_affect_state` + `affect_stance_lines` + `serialize_affect_state`) + impure `compute_assignment_affect_state` reader (`practice_analytics.py`); `analysis_state['affect_state']` snapshot; byte-identical when flag off or readiness neutral/settled; bounded nudge: never silences teacher-requested correction. Heuristic, NOT model-verified affect/WTC. Flag `PEDAGOGY_ENGINE_AFFECT` **BUILT, default `'0'`, NOT cut over** — flag absent/off in live prod. **S4.2:** L7 evidence-backed teacher debrief over `practice_sessions` still pending. | L3 (partial), L5 affect | both | reader only (no new store) |
