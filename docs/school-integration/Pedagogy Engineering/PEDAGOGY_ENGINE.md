...
|---|---|---|---|---|
| **S1 — Thin spine** ✅ BUILT 2026-06-22 (behind `PEDAGOGY_ENGINE_ASSIGNMENT_RENDER`, default off — not yet cut over; see `PEDAGOGY_ENGINE_S1.md` §10 + LIMITATIONS #53) | Reborn `backend/services/pedagogy/`: `compile_prompt_plan(bootstrap) -> PromptPlan` (§4.2a — targets + feedback_policy + task_context only) and `render_assignment_prompt(plan, surface)`. Re-house **only the assignment path** over the plan; **do not touch free practice**; prove zero prompt regression via snapshot; ship **one** visible behavior win (grammar-target slips escalate per `feedbackPolicy.mode`). Detailed in `PEDAGOGY_ENGINE_S1.md`. | L2 (thin), L5 (routing only) + render-helper | `system_prompt` (assignment only) | new backend module + tests + eval harness (not "zero" — see note) |
