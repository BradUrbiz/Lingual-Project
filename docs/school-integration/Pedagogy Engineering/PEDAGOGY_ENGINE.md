...
| **S3 — Conversation Sidecar / coach track** — **S3.1 shipped** (post-task review, **model-verified**); **S3.2** (live between-turn chips) / **S3.3** (promote-back + main tutor correction-light) / **S3.4** (Ask mode) **pending** (see `PEDAGOGY_ENGINE_S3.md`) | Parallel correction model + side-channel; L4 turn algorithm; promote-back via existing `feedbackPolicy`; L6 sidecar panel with Feedback / Ask modes (the multimodal text-support surface). Main tutor goes correction-light. **S3.1 delivers the foundation:** a fresh correction-model pass over the finished transcript → read-only post-task review panel on both surfaces (the first **model-verified** analysis; cached in `analysis_state['coach_review']`, generate-on-read, fail-open, flag `PEDAGOGY_ENGINE_COACH_REVIEW` — **CUT OVER 2026-06-23, default `1`**). | L4 (post-task), L6 (review panel), render | `conversation_sidecar` / `coach_track` | parallel model call + UI |
