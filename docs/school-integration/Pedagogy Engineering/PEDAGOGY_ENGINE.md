...
### 13.1 Dev harness (regression-tests prompt/pack behavior — cheap)
A **simulated-student** model (LLM at a defined proficiency + error profile) runs N scripted sessions against the tutor; an **LLM-as-judge** scores transcripts on a pedagogy rubric (mistake ID, guidance provision, output pushing, talk-time ratio, anti-sycophancy, target recycling, language appropriateness — BEA 2025 taxonomy). Gate: a pack/version beats the incumbent on ≥3 dimensions before promotion. ~$0.05/50 sessions at mini prices. **As built, this LLM behavioral layer is opt-in (`RUN_PEDAGOGY_EVAL=1`) and does NOT run in CI** — only the deterministic unit tests around the prompt/plan gate `make test-backend`. **This converts behavior from vibes to a dev metric; it does not prove learning.**
...
|---|---|---|---|---|
| **S1 — Thin spine** ✅ CUT OVER + LEGACY RETIRED 2026-06-23 (engine render is now **unconditional** — the `PEDAGOGY_ENGINE_ASSIGNMENT_RENDER` flag and the legacy `build_assignment_system_prompt` builder were deleted; rollback is revision-based; see `PEDAGOGY_ENGINE_S1.md` §10 + LIMITATIONS #53) | Reborn `backend/services/pedagogy/`: `compile_prompt_plan(bootstrap) -> PromptPlan` (§4.2a — targets + feedback_policy + task_context only) and `render_assignment_prompt(plan, surface)`. Re-house **only the assignment path** over the plan; **do not touch free practice**; prove zero prompt regression via snapshot; ship **one** visible behavior win (grammar-target slips escalate per `feedbackPolicy.mode`). Detailed in `PEDAGOGY_ENGINE_S1.md`. | L2 (thin), L5 (routing only) + render-helper | `system_prompt` (assignment only) | new backend module + tests + eval harness (not "zero" — see note) |
| **S2 — Closed loop** ✅ CUT OVER 2026-06-23 (`PEDAGOGY_ENGINE_RECYCLING=1` LIVE in prod, rev 00069, cloudbuild default `'1'`; gates only the route's prior-session/event reads — render is unconditional; rollback `=0`; see `PEDAGOGY_ENGINE_S2.md` + LIMITATIONS #53) | Coverage/evidence reader over existing `learning_events`; L3 learner-model accumulates **heuristic coverage tiers (hit-count proxy: not_attempted = 0 hits / emerging = 1–2 / solid = ≥3)** + repeated-error patterns (no affect yet) — count-based coverage evidence, **NOT model-verified mastery** (see LIMITATIONS #53(g)); feeds recycling + uncovered-target awareness back into S1's plan. Coverage tiering is **expression + vocabulary targets only** (they have real per-target hit maps in `session_summary`); grammar rules + objectives stay on S1 routing. The route coverage compute is **fail-open** (any reader/compute failure → no recycling, never a live-path 500). | L3 (partial), L2 coverage | `system_prompt` | reader only (no new store) |
| **S3 — Conversation Sidecar / coach track** — **S3.1 shipped** (post-task review, **model-verified**); **S3.2 CUT OVER 2026-06-24** (live behind `PEDAGOGY_ENGINE_COACH_CHIPS=1`, cloudbuild default `1`); **S3.3 BUILT behind flag** (`PEDAGOGY_ENGINE_PROMOTE_BACK=0`, default off, **pending cutover**); **S3.4 BUILT behind flag** (`PEDAGOGY_ENGINE_ASK_MODE=0`, default off, **pending cutover**) (see `PEDAGOGY_ENGINE_S3.md`) | Parallel correction model + side-channel; L4 turn algorithm; promote-back via existing `feedbackPolicy`; L6 sidecar panel with Feedback / Ask modes (the multimodal text-support surface). Main tutor goes correction-light. **S3.1 delivers the foundation:** a fresh correction-model pass over the finished transcript → read-only post-task review panel on both surfaces (the first **model-verified** analysis; cached in `analysis_state['coach_review']`, generate-on-read, fail-open, flag `PEDAGOGY_ENGINE_COACH_REVIEW` — **CUT OVER 2026-06-23, default `1`**). **S3.2 adds the live per-turn chip side-channel:** heuristic-gated (corrective signals at turn N/N+1), chips appended into `analysis_state['coach_chips']`, generated via `POST /api/practice-sessions/<id>/coach-chip`, voice chips at the between-turn breakpoint, main tutor unchanged (no promote-back — that's S3.3), **CUT OVER 2026-06-24** (`PEDAGOGY_ENGINE_COACH_CHIPS=1`, cloudbuild default `1`; en/fr/es have native feedback catalogs, ko/ru/he/tl fall back to generic-English). **S3.3 adds promote-back + correction-light:** pure `promote_back.py` decision (recurrence counter + mode-modulated thresholds + 3 guards: cooldown/per-session-cap/reset-on-promote); rides the existing S3.2 chip round-trip (no new endpoint, no 2nd LLM call); in-character injection via avatar-context pattern (voice) + next-turn `coachNote` (text); correction-light stance drops the correction ladder (two-flag safety invariant: `PEDAGOGY_ENGINE_PROMOTE_BACK` AND `PEDAGOGY_ENGINE_COACH_CHIPS`). Flag `PEDAGOGY_ENGINE_PROMOTE_BACK` **BUILT, default `'0'`, NOT yet cut over** — prompt byte-identical to today when off. **S3.4 adds Ask mode:** learner-initiated text-only quick help via `POST /api/practice-sessions/<id>/ask`; pure `ask.py` (`build_ask_prompt` anti-answer-dump contract + `parse_ask_answer` → `AskAnswer{answer, kind}`) + impure `ask_service.answer_ask`; `ask_log` on `analysis_state` SEPARATE from `learning_events` (help ≠ evidence); `ConversationSidecar` Feedback | Ask tab toggle; `askModeEnabled` on the launch bootstrap. Flag `PEDAGOGY_ENGINE_ASK_MODE` **BUILT, default `'0'`, NOT yet cut over** — `/ask` returns null and the Ask tab is hidden when off. Independent of the correction track and the S3.3 cutover. | L4 (post-task + between-turn + promote-back + ask), L6 (review panel + chip sidecar + Ask tab), render | `conversation_sidecar` / `coach_track` | parallel model call + UI |
| **S4 — Affect + debrief** — **S4.1 BUILT behind `PEDAGOGY_ENGINE_AFFECT` (default `'0'`), NOT yet cut over** (see `PEDAGOGY_ENGINE_S4.md`); **S4.2 BUILT behind `PEDAGOGY_ENGINE_DEBRIEF` (default `'0'`), NOT cut over** — S4 is now fully built behind flags (affect + debrief) | **S4.1:** coarse readiness heuristic (turn-length trend + repair density + abandonment) → `AffectState` → gentler L5 stance lines when strained; pure `affect.py` (stdlib-only: `compute_affect_state` + `affect_stance_lines` + `serialize_affect_state`) + impure `compute_assignment_affect_state` reader (`practice_analytics.py`); `analysis_state['affect_state']` snapshot; byte-identical when flag off or readiness neutral/settled; bounded nudge: never silences teacher-requested correction. Heuristic, NOT model-verified affect/WTC. Flag `PEDAGOGY_ENGINE_AFFECT` **BUILT, default `'0'`, NOT cut over** — flag absent/off in live prod. **S4.2:** pure stdlib-only `debrief.py` presenter `build_session_debrief(session_record) -> dict` projects `analysis_state` + `session_summary` into the L7 teacher debrief shape; read-only/no-LLM/no-store; `MAX_SUGGESTIONS=4`; static honesty `caveats` always present; `GET /api/teacher/practice-sessions/<session_id>/debrief` endpoint (session→assignment→class→teacher auth chain); `debriefEnabled` flag on three analytics payloads (assignment / class / student drill-down); frontend click-through to `SessionDebriefPage`. Flag `PEDAGOGY_ENGINE_DEBRIEF` **BUILT, default `'0'`, NOT cut over** — flag absent/off in live prod. | L3 (partial), L5 affect, L7 debrief | both | reader only (no new store) |
| **S5 — Director (gated)** | Between-turns `session.update` re-steer on drift — **only if** S1–S4 eval shows static composition plateaus below target. Eval instrument built 2026-06-24 (`backend/tests/eval/test_static_composition_drift_eval.py`; run with `RUN_PEDAGOGY_EVAL=1` to produce the gate verdict); **S5 remains deferred until a plateau is shown.** See `PEDAGOGY_ENGINE_S5.md`. Director BUILT behind `PEDAGOGY_ENGINE_DIRECTOR` (default off, 2026-06-24); detects **target-neglect + language-drift** (v2: language-drift takes precedence — non-Latin target-script ratio / Latin English-marker density; both pure heuristics, no LLM; delivery via `injectPromoteBack`/`coachNote`, NOT `session.update`); cutover still gated on the eval plateau verdict. | L4 runtime | `session.update` (intent; impl reuses `conversation.item.create`) | +latency/cost; prove first |
| **Eval harness** | Simulated-student + LLM-judge (§13.1) — built alongside **S1**, not after, so every slice's *prompt* is gated by **deterministic unit tests** (which DO gate `make test-backend`). The LLM behavioral layer is **opt-in via `RUN_PEDAGOGY_EVAL=1`** and is **NOT CI-gated** (no automatic LLM cost). | — | — | opt-in dev harness (NOT CI; deterministic units gate CI) |

**Governance** rides alongside as an extension of existing compliance services, not a slice. **L8 Teacher Preview** rides with S1 (the teacher must see the compiler's first inference). *Surfaced 2026-06-24:* `serialize_plan_preview` (built in S1) is now exposed via a read-only teacher endpoint `GET /api/teacher/assignments/<id>/plan-preview` + a "How the AI will run this" builder panel, behind `PEDAGOGY_ENGINE_TEACHER_PREVIEW` (default off) — shows the compiled base plan (engine on/off, task type, correction posture, target→feedback-route, raw-mode guarantees-disabled). **Free-chat parity** is deliberately *not* in S1 (codex #4) — it's a fast follow once the assignment path proves the plan→render contract, because free chat has no teacher targets and shouldn't gate the spine.

**On "net new infra":** S1 adds **no new persistence and no per-session latency/cost** — but it is not free. It's a new backend module, a migration of the live assignment prompt path, and new tests/snapshots. The eval harness splits in two: its **deterministic unit tests** are a real new code-surface dependency that gates `make test-backend`, while the **LLM behavioral layer is opt-in (`RUN_PEDAGOGY_EVAL=1`) and does NOT run in CI** — so there is no automatic per-build LLM cost. Calling S1 "zero infra" was the optimism codex flagged (§0.1 #4). The honest claim: *zero new datastores, real new code surface, and no CI-gated LLM cost.*
