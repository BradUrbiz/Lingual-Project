...
5. **classes-junction-backfill-prereq** *(HARD prereq — WRITES BUILT 2026-06-01, flag-gated, NOT yet deployed)* — `upsert_class` now populates `class_teachers` + `class_join_codes` (reconcile helpers, convergent); `shadow_create_class` inherits the teacher reconcile; `shadow_generate_class_join_code`/`shadow_deactivate_class_join_code` mirror the live join-code lifecycle (wired on `database.generate/deactivate_class_join_code` via `sql_engine=`). Tier-1 fakes updated + 6 gated Tier-2 PG tests. **REMAINING: (a) deploy the junction writes (still touches the deployed shadow-write path → deploy + live-verify); (b) re-run the reconciliation backfill (`--write`) so already-migrated classes get their junctions; (c) a junction-aware `parity_report`.** Only then is class-read (slice 6) correct. (Anchors TASKS.md line ~55.)
