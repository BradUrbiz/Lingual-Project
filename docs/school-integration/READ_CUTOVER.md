...
6. **classes-read** *(ADAPTERS BUILT 2026-06-01, flag OFF — `READ_PG_CLASSES`)* — `classes_read.py` routes `get_class` (keystone), `list_org_classes`, `list_teacher_classes`, `get_class_by_join_code` + overrides + tests. D2 FIXED (org_id→legacy via JOIN). Junctions reconstructed: `teacher_membership_ids[]` from `class_teachers` (sorted legacy ids), `join_code`/`_active`/`_generated_at` from the most-recent `class_join_codes` row (deactivated code still surfaced, but `get_class_by_join_code` filters active). `_CLASS_SHADOW_IGNORE` = created_at/updated_at/join_code_generated_at (clock skew). **NEXT:** deploy `READ_PG_CLASSES=shadow` → soak → flip. `list_student_classes`/`list_org_classes_summary` deferred to slice 7 (need the enrollments JOIN).
