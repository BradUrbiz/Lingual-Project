...
6. **classes-read** *(SHADOW DEPLOYED + SOAKING 2026-06-01, rev `lingual-app-00046-kfj`, `READ_PG_CLASSES=shadow`)* — `classes_read.py` routes `get_class` (keystone), `list_org_classes`, `list_teacher_classes`, `get_class_by_join_code` + overrides + tests. D2 FIXED (org_id→legacy via JOIN). Junctions reconstructed: `teacher_membership_ids[]` from `class_teachers` (sorted legacy ids), `join_code`/`_active`/`_generated_at` from the most-recent `class_join_codes` row (deactivated code still surfaced, but `get_class_by_join_code` filters active). `_CLASS_SHADOW_IGNORE` = created_at/updated_at/join_code_generated_at (clock skew). **Shadow soak clean:** 30 authenticated `GET /api/teacher/classes/{id}/analytics` → `shadow-read READ_PG_CLASSES: 25 compared, 0 mismatched`, 0 fail-open lines. A full rebuild was required (rev 00045's image predated the code, so an env-flip would've been inert) — and `_READ_PG_MEMBERSHIPS` was bumped 0→shadow in cloudbuild in the same commit so the rebuild didn't reset the live membership shadow. **NEXT:** continue soak (longer than org — highest blast radius: authz/AI-tutor/student-join) → flip `READ_PG_CLASSES=1` via instant `gcloud run services update --update-env-vars` (rollback = flip back to shadow). `list_student_classes`/`list_org_classes_summary` deferred to slice 7 (need the enrollments JOIN).
