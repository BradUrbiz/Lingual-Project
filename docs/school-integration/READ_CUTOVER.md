...
**Router-bypass audit (the deeper requirement — found in the org shadow soak, 2026-06-01).** The router only intercepts calls made through `deps.db`. Any caller that does `import database; database.get_organization(...)` **bypasses the router entirely** — harmless in `shadow` (Firestore is authoritative everywhere) but a **split-brain at `1`** (it keeps reading Firestore while routed callers read PG). Confirmed bypassers for `get_organization`: **`services/suspended_org_guard.py`** (`database.get_organization` — the fail-closed voice/practice gate, high traffic) and one path in **`services/compliance.py`** (`db.get_organization` via a direct module ref). Before flipping `READ_PG_ORGANIZATIONS=1`, migrate every direct-`database` reader of the entity to `deps.db` (pass the router into the service), or the gate evaluates suspension from a different store than the admin views. Repeat this bypass audit per entity. The shadow counters only cover routed reads, so a low compare-count relative to traffic is itself a signal that readers are bypassing.
...
| D4 | **`list_org_memberships` needs Firestore user data.** Output keys are `membership_id`/`uid`/`email`/`name`/`roles`/`status`/`joined_at`; callers do non-`.get()` `c['membership_id']`. `email`/`name` come from the `users` collection (**not in PG**) → a pure-SQL adapter is impossible; it must fan out to Firestore `get_user` per row (cross-store, N+1). Same for `list_school_admin_emails`. | critique-verified | Specify exact keys (`membership_id`, `joined_at` from `created_at`) + the per-row Firestore fan-out. **Decision needed:** the N+1 may negate the cutover benefit for these admin lists — consider leaving them on Firestore (orgName/orgType enrichment is a real PG JOIN; email/name are not). |
...
- **memberships** — `get_user_memberships` emits one dict per ROW with `orgName`/`orgType` (real JOIN to `organizations`) sorted by `_membership_sort_key`. **Merge-shape:** a multi-role user yields ONE dict with a unioned `roles[]` where Firestore yielded N (the 3 documented users). `resolve_user_school_context` stays a **Firestore wrapper** (it also reads the legacy `users.lingual_admin` flag + `users.profile`); only its membership-LIST leg delegates to PG. `list_lingual_admin_emails` stays dual-source (unions memberships with the legacy `users.lingual_admin` flag). `get_membership` by a lost-merge `{org}_{uid}` id resolves to no PG row → fails open to Firestore.
...
4. **memberships-read** + flag — keystone role-guard layer; the derived-value comparator (active-selection) must be green in shadow first; keep `resolve_user_school_context` wrapper on Firestore; resolve the D4 admin-list fan-out decision.
