...
- [-] Cut route reads over by family. **Scoped + designed + adversarially reviewed (2026-06-01) — authoritative sequencing in `READ_CUTOVER.md`.** Candidates are ONLY the 4 dual-written entities (org/membership/class/enrollment); the **analytics family is BLOCKED** (practice_sessions/learning_events are modeled but NOT dual-written/backfilled — see the `[!]` item below). Ordered slices:
  ...
  - [-] **(2) organizations-read** — adapter + **shadow DEPLOYED & SOAKED on prod** (2026-06-01, rev `lingual-app-00040+`, `READ_PG_ORGANIZATIONS=shadow`; reads still 100% Firestore). `organizations_read.get_organization` (full org shape, inverse `suspended_by_uid`/`restored_by_uid` renames, `school_admin_uids` omitted+allowlisted) + the `ReadRouter.get_organization` override. Verified end-to-end via 5 authenticated `GET /api/schools/teacher-invite-code` calls (school admin, org `fc53dfbc`): the router resolved the PG row + compared field-for-field; the **only** divergence was `teacher_invite_code_active: (None, False)` (Firestore-absent vs PG NOT-NULL default) — FIXED in the comparator's `_norm` (False≈absent). **Remaining before flipping `READ_PG_ORGANIZATIONS=1`:** (a) the other org readers for entity-atomicity (`search_organizations`, `list_organizations` incl. derived `school_admin_uids`/`memberCount` — defect **D3**, `count_organizations_by_status`, the missed `get_org_by_teacher_invite_code` — §3.0a); (b) **migrate the router-BYPASS callers to `deps.db`** — `suspended_org_guard.py` + one `compliance.py` path call `database.get_organization` directly and would split-brain at flag=1 (§3.0a router-bypass audit); (c) confirm a clean soak (`shadow-read … 0 mismatched`) then flip.
  - [ ] **(3) organizations COUNT aggregations** — ride `READ_PG_ORGANIZATIONS`.
  - [ ] **(4) memberships-read** + flag — keystone role-guard layer; derived-value comparator (active-membership selection after the merge) green in shadow first; keep `resolve_user_school_context` on Firestore; resolve the **D4** admin-list Firestore-`users` fan-out decision (may leave those lists on Firestore).
  - [ ] **(6) classes-read** + flag — highest blast radius (authz + prompt assembly + compliance + student join); fix defect **D2** (class `org_id` must emit the legacy id or `assignment_resolver.py:1289` authz fails). Depends on slice (5) below.
  - [ ] **(7) classes student/admin lists** + **(8) enrollments-read flip** — AFTER classes (roster, Canvas gap view, studentCount, practice-launch gate).
...
- [ ] **(5) classes-junction-backfill prereq (HARD)** — migrate teacher authz off `classes.teacher_membership_ids[]` to a `class_teachers` join (filtering `removed`) AND populate `class_join_codes`; both are currently deferred by `upsert_class`. Required before any class read (every PG `get_class` rebuilds `teacher_membership_ids[]`/join-code via these junctions). Populate in the live dual-write path AND a reconciliation backfill with its own `parity_report` — **touches the deployed shadow-write path, so it needs a deploy + verification; its own slice.**
