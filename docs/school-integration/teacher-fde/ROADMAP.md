...
| 2026-06-28 | **Uptake trace BUILT + CUT OVER + LIVE** (`PEDAGOGY_ENGINE_UPTAKE_TRACE=1`, rev 00095-6rw) — closes the synthetic pre-validation finding #4 (productive-struggle purist's trust condition): classifies each realized production as after-prompt / after-recast / unprompted, resolving the hit-count's parroting-vs-production ambiguity. Path A (read-time derive from persisted events, no live-path change). **Prod runtime-verified** on the burn-in assignment (`measured=4`, unprompted 3 / afterRecast 1). | The strongest convergent refute after prose-extraction; derivable cheaply from data already persisted; lit up today's real burn-in on first cutover. |
| 2026-06-28 | **Prose-extraction spike → hypothesis SUPPORTED** (`PHASE2_PROSE_EXTRACTION_SPIKE.md`): tested finding #1 on Polón's REAL prose with the real text-LLM pattern. Lexical core extracts faithfully + degrades to clarifying questions on thin input; the one boundary (`focus_grammar`) is fixed by a curriculum map (Imagina Lección→Estructuras) the prose already references. | De-risks the biggest Phase 2 reshape before the real build; gives Phase 2 a validated authoring shape (paste-prose → extract → confirm) + a reusable extraction contract. Still pre-sign-off (UX/time-on-task unproven). |
