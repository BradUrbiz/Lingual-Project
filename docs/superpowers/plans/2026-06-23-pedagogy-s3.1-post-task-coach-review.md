...
### Task 6: Mount the review launcher in `AssignmentPracticeWorkspace.tsx`

Offer the review **after** an assignment practice session ends, in the component that actually owns the session.

> **CORRECTION (controller, during execution):** the assignment practice session lives in `frontend/src/components/assignments/AssignmentPracticeWorkspace.tsx` — **not** `AppChatPage.tsx`, which is the legacy B2C chat surface with no assignment session. Mount here. Also: the workspace does **not** locally flip a session's `status` to `'completed'` on end (that only happens when a *new* session is merged in, ~lines 66/71-72), so there is no reliable render-time "ended" status signal. Capture the ending session id explicitly instead (below). This guarantees the review is offered only after an explicit end — never mid-session — so generate-on-read can't cache a partial transcript.
...
**Interfaces:**
- Consumes: `PostTaskReviewPanel`; the workspace's existing `handleEndSession` (line 961 — the "End session" finish path), `selectedActivePracticeSession` (`{id, chatId, status, ...}`), `useRealtimeChat().isConnected` (already destructured), and the `AssignmentConversationView` child + its props interface.
- Produces: the `ReviewLauncher` component (props `{sessionId, canReview, label}`) + a `reviewSessionId` local state in the workspace, set to the ending session's id when the learner ends the session.

> **Implementation guidance (AssignmentPracticeWorkspace is large + reducer-based; the conversation UI is a CHILD component `AssignmentConversationView` — change surgically; the testable open/close logic lives in the extracted `ReviewLauncher`, Step 3):**
> - Import `ReviewLauncher` into the file.
> - In the top-level `AssignmentPracticeWorkspace` component, add local state `const [reviewSessionId, setReviewSessionId] = useState<string | null>(null);`.
> - **Trigger = `handleEndSession` (line 961), the explicit "End session" finish path** (it already reads `selectedActivePracticeSession`, fires `session.ended`, `disconnect()`s, then `loadWorkspace(...)`). Capture the id at the TOP of the function (before the awaits/reload, while it's still non-null) and set the review state:
>   ```tsx
>   const handleEndSession = async () => {
>     if (!selectedActivePracticeSession) return;
>     const endedId = selectedActivePracticeSession.id;   // capture before reload nulls it
>     dispatch({ type: 'patch', payload: { isMutating: true, error: null } });
>     try {
>       // ...existing clearRealtimePersistenceTarget / reportPracticeSessionEvent / clearMessages / disconnect / loadWorkspace...
>       setReviewSessionId(endedId);
>     } catch (endError) { /* ...existing... */ }
>     finally { /* ...existing... */ }
>   };
>   ```
>   Do NOT also wire `endActivePracticeSession` (restart/thread-resume) — those abandon-to-restart and are not reviewable in this slice. `handleEndSession` is the only setter.
> - Compute `const canReview = !!reviewSessionId && !isConnected;` (after End session, `disconnect()` makes `isConnected` false; the gate also prevents the launcher from showing during a live voice session).
> - The launcher renders inside the `AssignmentConversationView` child (its `return` is ~line 594). Thread two new props into that child's props interface + its call site: `reviewSessionId: string | null` and `canReview: boolean`. Render the launcher just below `<AssignmentMessagesPane .../>` (~line 602):
>   ```tsx
...
>   ```
> - Use the workspace's actual identifiers (grep to confirm: `selectedActivePracticeSession`, `isConnected`, `handleEndSession`, `AssignmentConversationView`); do not invent new ones. Keep the prop-threading minimal (just the two new props).

- [ ] **Step 1: Write the failing test**

The testable open/close logic is the extracted, file-agnostic `ReviewLauncher` (the host workspace only computes `canReview`/`sessionId` and renders it). Write the launcher's test first.
...
Create the `ReviewLauncher.tsx` shown above. Then wire it into `AssignmentPracticeWorkspace.tsx` exactly per the Implementation-guidance block above: import `ReviewLauncher`; add `reviewSessionId` state in the top-level component; set it inside `handleEndSession` (capture `selectedActivePracticeSession.id` at the top, before the reload nulls it); compute `canReview = !!reviewSessionId && !isConnected`; thread `reviewSessionId` + `canReview` as two new props into the `AssignmentConversationView` child + its props interface; render `<ReviewLauncher sessionId={reviewSessionId} canReview={canReview} label="See coach review" />` just below `<AssignmentMessagesPane />`. Use the workspace's real identifiers — do not invent new ones.
...
```bash
git add frontend/src/components/learning/ReviewLauncher.tsx frontend/src/components/learning/ReviewLauncher.test.tsx frontend/src/components/assignments/AssignmentPracticeWorkspace.tsx
git commit -m "feat(pedagogy-s3): mount post-task review launcher in AssignmentPracticeWorkspace (session-end trigger)"
```
